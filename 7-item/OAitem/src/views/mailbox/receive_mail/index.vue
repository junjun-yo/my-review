        ...
        <div style="margin:20px auto 10px 10px">
            ...
            <el-dropdown style="margin-left: 10px">
                ...
                <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item @click.native="setFileType(1)">已读</el-dropdown-item>
                    <el-dropdown-item @click.native="setFileType(0)">未读</el-dropdown-item>
            ...
            <el-button type="primary" style="margin-left: 10px" @click="downloadExcel">导出</el-button>
        </div>

        <div>
            <oa-table :ajax="ajaxParams"
                      :columns="columns"
                      showCheck showPage
                      @check="onCheck"
                      ref="table"
...
<script>
    export default {
        ...
        methods:{
            ...
            NewAndMove(){
                ...
                this.$prompt('请输入文件夹名称','提示',{
                    ...
                    for(let i=0;i<this.checks.length;i++){
                        this.checks[i].mailReceiverDir = value.value;
                    }
                    ...
                    for(let i=0;i<this.checks.length;i++){//保存到文件夹
                        this.params.push({
                            ...
                            mailReceiverId:this.checks[i].mailReceiverId,
                            mailId:""
                        });
                    }
            ...
            moveTo(dir){
                for(let i=0;i<this.checks.length;i++){
                    this.checks[i].mailReceiverDir = dir.dirName;
                }
                //对获得的id进行处理
                Ajax.put("/mailbox/updateMailInfo",this.checks,"json").then(()=>{
                for(let i=0;i<this.checks.length;i++){//保存到文件夹
            ...
            change(){
                this.$router.push({path:'/mailBox/my_dir'})
            }
        },
        created() {
